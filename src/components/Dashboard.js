...
export const Dashboard = () => {
  ...
  const { saveRigBuild, getRigBuildById, updateRigBuild } =
    useContext(RigBuildContext);
  ...
  const addToArray = (e) => {
    ...
    newArray.splice(
      whichDrop,
      1,
      graphicsCards.find((gc) => gc.id === +e.target.value)
    );
  ...
  const calculateRevenue = (totalHashrate) => {
    ...
    const rigBuildHashrateShare = +(
      totalHashrate /
      (ethData[0].network_hashrate / 1000000)
    );
    const dailyBlockReward = +(
      ethData[0].reward_block *
      (SECONDS_PER_DAY / ETH_BLOCK_TIME)
    );
  ...
  const handleSaveEvent = () => {
    if (cost === 0 || hashrate === 0 || powerConsumption === 0) {
      ...
      saveRigBuild({
        ...
        motherboardId: motherboards.find(
          (mobo) => mobo.num_of_cards_supported === +dropzoneSize
        ).id,
        ...
        .then((savedRigBuild) => {
          ...
          Promise.all(
            joinTableArray.map((singleJoinObj) => {
            ...
            <div className="build-area-totals">
              <div className="crypto-logo">
                <img
                  ...
                  src={require("../imgs/ethereum-logo-2.png")}
              ...
              <div className="rig-build-btns">
                ...
                >
                  <span>
                    Build<i class="bi bi-hammer build-rig-icon"></i>
                  </span>
                </button>
                <button
                ...
                >
                  <span>
                    Save<i class="bi bi-cloud-download save-rig-icon"></i>
                  </span>
                </button>
              </div>
            </div>
          ...
          <div className="calculation-metrics__container">
            <div className="revenue">
              ...
              <div className="revenue-metrics">
                <p>
                  <span>${revenue[0]}</span> /day
                </p>
                <p>
                  <span>${revenue[1]}</span> /week
                </p>
                <p>
                  <span>${revenue[2]}</span> /month
                </p>
                <p>
                  <span>${revenue[3]}</span> /year
                </p>
              </div>
            </div>
            <div className="electricity">
              <h2>
                Electricity Costs
                <span className="metric-heading-span">($0.13/kWh)</span>
              </h2>
              <div className="electricity-metrics">
                <p>
                  <span>${electricity[0]}</span> /day
                </p>
                <p>
                  <span>${electricity[1]}</span> /week
                </p>
                <p>
                  <span>${electricity[2]}</span> /month
                </p>
                <p>
                  <span>${electricity[3]}</span> /year
                </p>
              </div>
            </div>
            <div className="profit">
              <h2>
                Profit
                <span className="metric-heading-span">
                  (revenue - electric cost)
                </span>
              </h2>
              <div className="profit-metrics">
                <p>
                  <span>${profit[0]}</span> /day
                </p>
                <p>
                  <span>${profit[1]}</span> /week
                </p>
                <p>
                  <span>${profit[2]}</span> /month
                </p>
                <p>
                  <span>${profit[3]}</span> /year
                </p>
              </div>
            </div>
          </div>
